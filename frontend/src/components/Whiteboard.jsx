...
export default function Whiteboard(){
  ...
  // Build WS base (if deployed single-service, empty => same origin)
  const BACK = import.meta.env.VITE_BACKEND_URL || "";
  const wsBase = BACK ? BACK.replace(/^http/, 'ws') : `${location.protocol === 'https:' ? 'wss' : 'ws'}://${location.host}`;
  ...
  useEffect(() => {
    ...
    try {
      wsRef.current = new WebSocket(`${wsBase}/ws/${roomId}`);
  ...
  async function doAIClean(){
    ...
    const BACK = import.meta.env.VITE_BACKEND_URL || "";
    const res = await fetch(`${BACK}/api/ai/clean`, { method: "POST", body: fd });
